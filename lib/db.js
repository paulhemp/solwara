...
async function ensureSchema(sql) {
  ...
  await sql`CREATE TABLE IF NOT EXISTS orders (
    ...
    item_total NUMERIC, shipping NUMERIC, items TEXT,
    customer_name TEXT, customer_email TEXT, shipping_address TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
  )`
  // Safe to run on tables created by an earlier version.
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS item_total NUMERIC`
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping NUMERIC`
...
export async function recordOrder(o) {
  ...
  const rows = await sql`
    INSERT INTO orders
      (paypal_order_id, status, amount, currency, item_total, shipping, items, customer_name, customer_email, shipping_address)
    VALUES
      (${o.orderId}, ${o.status}, ${o.amount || null}, ${o.currency},
       ${o.itemTotal || null}, ${o.shippingTotal || null}, ${o.items || ''},
       ${o.name}, ${o.email}, ${o.address})
...
export async function getAllData() {
  ...
  const [subscribers, contacts, wholesale, orders] = await Promise.all([
    ...
    sql`SELECT id, paypal_order_id, status, amount, currency, item_total, shipping, items, customer_name, customer_email, shipping_address, created_at FROM orders ORDER BY created_at DESC LIMIT 5000`,
  ])
